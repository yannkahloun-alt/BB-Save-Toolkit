...
def _static_response(name: str, content_type: str) -> HttpResponse:
    """Serve only a fixed application asset; never map request paths to disk."""
...
class LocalApplicationApi:
    ...
    def handle(
        ...
        try:
            ...
            if method == "GET" and path.startswith(assigned_prefix):
                ...
                if len(parts) != 2:
                    raise ApplicationOperationError(
                        "invalid_request",
                        "assigned-build read requires campaign and entity tokens",
                    )
                return self._ok(
                    self.application.assigned_build(
            ...
            if method == "GET" and path.startswith("/api/v1/analysis/jobs/"):
                job_id = self._positive_integer(
                    path.rsplit("/", 1)[-1], "job id"
                )
            ...
            if path == "/api/v1/followed-save/select":
                self._exact_keys(
                    payload, {"path", "expected_revision"}, {"auto_refresh"}
                )
            ...
            if path == "/api/v1/followed-save/forget":
                ...
                return self._ok(
                    self.application.forget_followed_save(
                        expected_revision=self._revision(payload)
                    )
                )
            if path == "/api/v1/analysis/jobs":
                ...
                revision = self._integer(
                    payload["expected_preferences_revision"],
                    "expected_preferences_revision",
                )
                return self._ok(
                    self.application.request_analysis(
            ...
            if path.startswith(prefix):
                ...
                return self._ok(
                    self.application.mutate_archetypes(operation, payload)
                )
            if path.startswith(assigned_prefix):
                ...
                return self._ok(
                    self.application.mutate_assigned_build(operation, payload)
                )
            return self._error(404, "not_found", "endpoint was not found")
            ...
            return self._error(
                status, exc.code, exc.message, details=exc.details
            )
            ...
            return self._error(
                422, "validation_failed", str(exc), details=details
            )
            ...
            # Keep the bounded service alive without reflecting private
            # payloads or filesystem details to the browser.
            return self._error(
                500, "internal_error", "the application operation failed"
            )

    def _shell_state(self) -> dict[str, Any]:
        """Compose the shell's read-only status model from authoritative state."""
    ...
    def _validate_host(self, headers: Mapping[str, str]) -> None:
        ...
        if headers.get("host") != authority:
            raise ApplicationOperationError(
                "invalid_host", "request Host is not the local application"
            )
    ...
    def _mutation_payload(
        ...
        if method != "POST":
            raise ApplicationOperationError(
                "method_not_allowed", "only POST is allowed for mutations"
            )
        if headers.get("origin") != self.origin:
            raise ApplicationOperationError(
                "untrusted_origin", "mutation Origin is not trusted"
            )
        if not secrets.compare_digest(
            headers.get("x-bbst-session", ""), self.token
        ):
            raise ApplicationOperationError(
                "invalid_session", "mutation session capability is invalid"
            )
        if (
    ...
    def _validate_archetype_command(
        ...
        if operation not in shapes:
            raise ApplicationOperationError(
                "unknown_operation", "unknown archetype operation"
            )
        ...
        if "id" in payload and not isinstance(payload["id"], str):
            raise ApplicationOperationError(
                "invalid_request", "id must be a string"
            )
        if "disabled" in payload and not isinstance(
            payload["disabled"], bool
        ):
            raise ApplicationOperationError(
                "invalid_request", "disabled must be boolean"
            )
        if "merge" in payload and not isinstance(payload["merge"], bool):
            raise ApplicationOperationError(
                "invalid_request", "merge must be boolean"
            )
        if "patch" in payload and not isinstance(payload["patch"], dict):
            raise ApplicationOperationError(
                "invalid_request", "patch must be an object"
            )
        if "definition" in payload and not isinstance(
            payload["definition"], dict
        ):
            raise ApplicationOperationError(
                "invalid_request", "definition must be an object"
            )
        if "document" in payload and not isinstance(
            payload["document"], str
        ):
            raise ApplicationOperationError(
                "invalid_request", "document must be a string"
            )

    def _validate_assigned_build_command(
        self, operation: str, payload: dict
    ) -> None:
        identity = {
            "campaign_identity",
        ...
        shapes = {
            ...
            "clear": (identity, set()),
            "clear_campaign": (
                {"campaign_identity", "expected_revision"},
                set(),
            ),
        }
        if operation not in shapes:
        ...
        self._exact_keys(payload, required, optional)
        payload["expected_revision"] = self._revision(payload)
        payload["campaign_identity"] = self._integer(
            payload["campaign_identity"], "campaign_identity"
        )
        if "native_entity_token" in payload:
            token = self._integer(
                payload["native_entity_token"], "native_entity_token"
            )
            if token == 0 or token > 0xFFFFFFFF:
                raise ApplicationOperationError(
                    "invalid_request",
                    "native_entity_token must be a non-zero unsigned 32-bit integer",
                )
            payload["native_entity_token"] = token
        if "build_identity" in payload and not isinstance(
            payload["build_identity"], str
        ):
            raise ApplicationOperationError(
                "invalid_request", "build_identity must be a string"
            )
    ...
    def _ok(data: Any, *, status: int = 200) -> HttpResponse:
        return _json_response(
            status, {"schema": API_SCHEMA, "data": data, "error": None}
        )
    ...
    def _error(
        ...
        *,
        ...
        return _json_response(
            status, {"schema": API_SCHEMA, "data": None, "error": error}
        )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            body = self.rfile.read(
                min(max(length, 0), MAX_REQUEST_BYTES + 1)
            )
            self._dispatch(body)
...
def serve_local_application(
    *,
    ...
    config = load_config(
        config_root / "archetypes.json",
        config_root / "classification.json",
    )
    ...
    server = ThreadingHTTPServer(
        (LOOPBACK_HOST, port), BaseHTTPRequestHandler
    )
